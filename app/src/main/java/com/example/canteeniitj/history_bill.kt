...
class history_bill : AppCompatActivity() {
...
//        val path= (getExternalFilesDir(null)?.absolutePath ?: null) + "/" + ".jpg"
//        val x=findViewById<LinearLayout>(R.id.main_layout)
//        var bitmap = Bitmap.createBitmap(x.width,x.height,Bitmap.Config.ARGB_8888)
...
//        val URI=FileProvider.getUriForFile( applicationContext,)
        ...
        val geti=intent.getStringExtra("order_id")
        var listkeys:MutableList<String> = mutableListOf()
        var demo:HashMap<String,HashMap<String,Int>>
        var hashi:HashMap<String,Int> = hashMapOf()
        var total:Int=0
        Firebase.firestore.collection("Users").document(FirebaseAuth.getInstance().getCurrentUser()?.getUid().toString()).get().addOnSuccessListener {
            ...
            hashi=demo[geti] as HashMap<String, Int>
            listkeys=ArrayList(hashi.keys)
            for(i in hashi){
                total += data.get(i.key)?.get("Price")?.toInt()!! * i.value
            }
            Log.d(TAG,"${total}")
            findViewById<TextView>(R.id.tota_billi).text="Total Bill - "+total.toString()
            val hehe=findViewById<RecyclerView>(R.id.recycler_for_history_bill)
            val hehe1=adaptor1_for_history(this,listkeys,hashi,data,total)
            hehe.layoutManager=LinearLayoutManager(this,LinearLayoutManager.VERTICAL,false)
            hehe.adapter=hehe1
