...
class favourites : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<RecyclerView>(R.id.recycler_for_fav_items).alpha=1F
        ...
        db.collection("Users").document(FirebaseAuth.getInstance().getCurrentUser()?.getUid().toString()).get().addOnSuccessListener {
            val list_of_fav = it.data?.get("favs") as List<Int>
            val x=findViewById<RecyclerView>(R.id.recycler_for_fav_items)
            val y=adaptor_for_fav(this,list_of_fav,data)
