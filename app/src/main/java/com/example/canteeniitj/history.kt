...
class history : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dialog.findViewById<TextView>(R.id.message).text="Showing Orders..."
        val anim=dialog.findViewById<LottieAnimationView>(R.id.history)
        anim.playAnimation()
        anim.visibility= View.VISIBLE
        dialog.findViewById<ProgressBar>(R.id.progiii).visibility= View.GONE
        dialog.show()
        findViewById<RecyclerView>(R.id.recycler_for_history).alpha=0.5F
        anim.addAnimatorListener(object :Animator.AnimatorListener{
            ...
            override fun onAnimationEnd(p0: Animator) {
                findViewById<RecyclerView>(R.id.recycler_for_history).alpha=1F
                dialog.hide()
            }
        ...
        var deta:HashMap<String,Map<String,String>> = hashMapOf()
        deta= hashMapOf(
        ...
        var datetime:List<String> = listOf("13:44:15 15-07-2023")

        var keyList:List<String> = listOf()
        var data:HashMap<String,HashMap<String,Int>>
        Firebase.firestore.collection("Users").document(FirebaseAuth.getInstance().getCurrentUser()?.getUid().toString()).get().addOnSuccessListener {
            data=it.data?.get("orders") as HashMap<String, HashMap<String, Int>>
                keyList = ArrayList(data.keys)

            val formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm:ss")

            // Convert the strings to LocalDateTime objects and sort them in descending order
            val sortedDateTimeStrings = keyList
                .map { LocalDateTime.parse(it, formatter) }
                .sortedDescending()
                .map { it.format(formatter) }

            keyList = sortedDateTimeStrings
                val x=findViewById<RecyclerView>(R.id.recycler_for_history)
                val y=adaptor_for_history(keyList,deta)
