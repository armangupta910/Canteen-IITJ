...
class profile : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        db111.collection("Users").document(FirebaseAuth.getInstance().getCurrentUser()?.getUid().toString()).get().addOnSuccessListener {
            if(it.data?.get("URL")==""){
        ...
        button.setOnClickListener {
            ...
            if(imagi==null){
                ...
                storageref.getReference("Profile_Images").child(firebaseAuth.getCurrentUser()?.getUid().toString()).putFile(
                    imagi!!
                ).addOnSuccessListener {
                    Toast.makeText(this,"Image Uploaded",Toast.LENGTH_SHORT).show()
                    it.metadata!!.reference!!.downloadUrl.addOnSuccessListener {
                        val imageurl=it
                        db.collection("Users").document(firebaseAuth.getCurrentUser()?.getUid().toString()).update("URL",imageurl).addOnSuccessListener {
            ...
            button.setOnClickListener {
                ...
                db.collection("Users").document(firebaseAuth.getCurrentUser()?.getUid().toString()).update(
